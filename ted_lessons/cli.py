...
def cmd_migrate(args):
    ...
    # Collapse duplicates by lesson_id using deterministic "best row" rules:
    #   1. Prefer transcript_status == "ok"
    #   2. Then prefer scrape_status == "ok"
    #   3. Then prefer more non-empty content fields
    #   4. Then prefer newer last_enriched timestamp
    #   5. Always preserve successful transcript from losing row
    ...
    for lesson in lessons:
        ...
        if lid in deduped:
            ...
            winner = _pick_best_row(existing, lesson)
...
def _pick_best_row(a: Lesson, b: Lesson) -> Lesson:
    ...
    a_count = _content_field_count(a)
    b_count = _content_field_count(b)
    if a_count != b_count:
